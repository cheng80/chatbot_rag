...
계룡시 1장은 실제 저커버리지 지역으로 보고 필요하면 별도 보강한다.
...
전국 실사용 지역 목표는 250개다. 현재 TourAPI 캐시는 234개 시군구 기준이고, 제품 목표에는 행정시/일반구처럼 사용자가 지역명으로 묻는 단위가 더 포함될 수 있다.
...
2. 품질 QA에서 빈 지역이나 음식점/숙박 과다 지역이 드러나면 해당 지역만 보강한다.
3. 전국 실사용 250개 기준 대상 목록은 별도로 확장한다.
4. 전국 시군구별 5장 이상은 하루에 끝내지 않고 quota window별로 분할한다.
