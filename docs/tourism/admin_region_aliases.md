...
- bare 일반구 표현이 여러 부모 시군구로 매핑되면 바로 확정하지 않고 지역 선택 질문
- `고양 일산동구에서 휠체어 관광지 추천` -> 경기 고양시 TourAPI 코드
- `상동에서 유모차 관광지 추천`처럼 동명이 법정동/행정동이 여러 시군구로 매핑되면 지역 선택 질문
- `부천 상동에서 유모차 관광지 추천`처럼 부모 도시가 함께 있으면 해당 시군구로 확정
