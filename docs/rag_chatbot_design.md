...
- 문서에 없는 내용은 추측하지 않는다.
- 답변과 함께 출처를 반환한다.
- FastAPI 서버로 구성해 Web, App, Flutter 클라이언트에서 호출할 수 있게 한다.
```
...
```text
1. docs/rag_chatbot_design.md 존재
...
9. Ollama 모델 준비 완료: qwen3:8b, qwen3-embedding
10. scripts/ingest_all.py 실행 성공
    - document_count: 1
    - chunk_count: 1
    - collection_name: manual_documents
11. uvicorn app.main:app --reload 실행 및 /health 응답 확인
    - 응답: {"status":"ok"}
```

주의할 점은 `.venv`가 `/opt/anaconda3/bin/python`을 기반으로 만들어졌더라도, 실행 시 venv가 활성화되지 않으면 전역 Anaconda Python을 사용할 수 있다는 것이다. 서버와 스크립트 실행은 `.venv/bin/python -m ...` 형식을 쓰면 가장 명확하다.

응답 속도 기준으로는 Chroma 검색보다 Ollama 답변 생성 시간이 대부분을 차지한다. `qwen3:8b` 사용 시 `LLM_THINK=false`를 기본값으로 두고, `LLM_NUM_PREDICT`로 최대 생성 토큰을 제한한다.
