...
- 사용자가 자연어로 지역과 조건을 말하면 관광지 3~5개를 추천한다.
- 각 추천에는 접근성 또는 가족 친화 근거가 포함된다.
- 관광지 카드에는 출처, 주소, 이미지, 전화번호, 핵심 편의정보가 표시된다.
- 문서나 API에 없는 내용은 추측하지 않는다.
- 시군구 결과가 부족할 때 의미 없는 타 지역 추천을 자동으로 섞지 않는다. 사용자가 “근처/주변/가까운/인근”처럼 확장 의도를 밝힌 경우에만 상위 광역 지역으로 넓힌다.
...
- 2026-05-15 기준 서울/부산/강릉 각 5건, 총 15건의 live 무장애 Markdown 수집에 성공했다.
- curated 무장애·가족 친화 Markdown 샘플은 API 실패 대비와 테스트 fallback으로 유지한다.
- 2026-05-15 기준 시군구 추천 정책을 반영했다. 강남구 단독 질문은 강남구 결과 2건과 부족 안내를 반환하고, 강남구 근처 질문은 서울 범위 확장 안내와 함께 5건을 반환한다.
