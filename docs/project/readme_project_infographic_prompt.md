...
- 친근한 손그림풍 라인 일러스트와 보기 좋은 장식 밀도
- 사용자, 가족, 휠체어 이용자, 스마트폰, 관광 카드, 지도, 서버, 데이터베이스, 돋보기 아이콘
- Ollama는 공식 로고 복제가 아니라 귀여운 로컬 AI 라마 마스코트 느낌으로 작게 표현
- README 대표 이미지 느낌
- 카드와 섹션이 명확하지만, 웹페이지 스크린샷처럼 보이지 않게 해
- 사람 일러스트는 모두 목, 몸통, 팔, 다리 또는 휠체어가 연결된 전신 캐릭터로 그려. 머리만 떠 보이는 표현은 금지
- 가족/휠체어 이용자 일러스트는 바닥선 위에 안정적으로 배치
- 하트 아이콘은 사람 머리 위에 붙이지 말고, 필요하면 작은 배경 장식으로 멀리 배치하거나 생략
