...
| 명시적 범위 확장 허용 | `서울 강남구 근처...` -> 강남구 우선, `서울 전체로 넓혀줘` -> 서울 범위 포함 | 일상적 근처/주변 표현과 광역 확장을 분리 |
...
8. **확장 계획 명확성**: TourAPI 234개 시군구, 행안부 매칭 228개, 제품 상한 250개 표현, 주기적 갱신, SQLite 로그 전환, 모델 비교가 TODO로 분리돼 있다.
...
- TourAPI 234개 시군구와 행안부 매칭 228개 차이를 기준으로 예외 지역을 정리하고, 제품 상한 250개 표현은 실제 질문 빈도에 따라 별도 확정한다.
