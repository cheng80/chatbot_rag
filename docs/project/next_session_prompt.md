...
- 경계가 애매한 문장은 단정하지 않고 추가질문한다. `서울에서 계단 적게 다니는 편한 관광지`처럼 고령자/접근로/휠체어 중 의도가 불명확하면 `ambiguous_conditions`를 채우고 `/tourism/chat`은 `lookup_mode=clarification`으로 응답한다. `휠체어`, `경사로`, `접근로`, `출입통로`, `어르신`, `부모님`, `무릎`, `쉬어`, `앉`처럼 사용자가 기준을 명시한 경우는 바로 검색한다. 고령자 조건은 원천 데이터에 전용 필드가 부족하므로 카드 이유에 휠체어 접근, 경사로, 화장실, 대중교통 같은 이동 편의 근거를 함께 확인한다고 명시한다. 이 정책과 `대중교통` 단독 예외 보정 뒤 residual hard chat은 80/80으로 통과했다. residual hard NLU current policy는 rule/parser exact 0.8240/micro-F1 0.9184, RoBERTa 단독 exact 0.9360/micro-F1 0.9549다.
