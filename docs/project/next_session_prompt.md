...
- 실제 `/tourism/chat` 기준으로도 확인했다. 중간 길이 v4 120건은 기본 rule/parser와 hard 증강 RoBERTa 보조가 모두 100/120 통과로 회귀가 없었다. hard 의미 chat 40건은 기본 rule/parser 21/40 통과, hard 증강 RoBERTa 보조 36/40 통과다. 이 시점에는 hard 증강 모델을 로컬 후보로 적용했지만, 다음 잔여 증강 결과가 더 좋아 현재 적용 경로는 아래 잔여 증강 모델로 바뀌었다. 모델 산출물은 git ignore 대상이라 새 환경에서는 학습 명령으로 다시 만들어야 한다.
- 이후 남은 실패인 `고령자`, `주챠` 같은 주차 오타, `접근로`/`휠체어` 경계 혼동을 별도로 보기 위해 `data/eval/tourism_residual_hard_nlu_20260518.jsonl` 250건과 `data/eval/tourism_residual_hard_chat_20260518.jsonl` 80건을 만들었다. hard 증강 RoBERTa 단독은 잔여 NLU exact 0.7440/micro-F1 0.8560이었고, 잔여 증강 RoBERTa 단독은 exact 0.9360/micro-F1 0.9549로 개선됐다. 실제 chat은 기본 rule/parser 15/80, hard 증강 RoBERTa 53/80, 잔여 증강 RoBERTa 62/80 통과다. 중간 길이 v4 120건은 잔여 증강 RoBERTa도 100/120으로 회귀가 없고, hard 의미 chat 40건은 36/40으로 유지됐다. 로컬 적용 경로는 `TOURISM_CONDITION_TRANSFORMER_MODEL=./data/generated/tour_api/condition_transformer_residual_aug_e2_fast/model`, `TOURISM_CONDITION_TRANSFORMER_METRICS_PATH=./data/generated/tour_api/condition_transformer_residual_aug_e2_fast/metrics.json`로 갱신했다.
- `주차말고`, `유모차 말고 휠체어`처럼 조건 자체를 빼는 문장은 `excluded_conditions`로 분리해 rule/parser 조건과 Transformer 보조 라벨에서 모두 제거한다. 이 보강 뒤 residual NLU에서 rule/parser exact는 0.6640 -> 0.8240, rule+RoBERTa exact는 0.7720 -> 0.8240으로 개선됐다. 이후 `주챠`, `주차ㅏ`, `주자창` 같은 주차 오타를 로컬 정규화/조건 사전에 반영하고, 같은 라벨에서 제외가 포함보다 우선하도록 바꿨다.
- 경계가 애매한 문장은 단정하지 않고 추가질문한다. `서울에서 계단 적게 다니는 편한 관광지`처럼 고령자/접근로/휠체어 중 의도가 불명확하면 `ambiguous_conditions`를 채우고 `/tourism/chat`은 `lookup_mode=clarification`으로 응답한다. `휠체어`, `경사로`, `접근로`, `출입통로`, `어르신`, `부모님`, `무릎`, `쉬어`, `앉`처럼 사용자가 기준을 명시한 경우는 바로 검색한다. 단, `어머니`, `아버지`, `엄마`, `아빠` 같은 부모 호칭만으로는 고령자라고 단정하지 않는다. `오래안걷`, `무릎`, `쉬어` 같은 이동 부담 표현이 함께 있을 때 고령자 조건으로 본다. 고령자 조건은 원천 데이터에 전용 필드가 부족하므로 카드 이유에 휠체어 접근, 경사로, 화장실, 대중교통 같은 이동 편의 근거를 함께 확인한다고 명시한다. 이 정책과 `대중교통` 단독 예외 보정 뒤 residual hard chat은 80/80으로 통과했다. residual hard NLU current policy는 rule/parser exact 0.8240/micro-F1 0.9184, RoBERTa 단독 exact 0.9360/micro-F1 0.9549다.
- 2026-05-18 noisy realistic chat 200건을 추가했다. 파일은 `data/eval/tourism_noisy_realistic_chat_eval_v1_200.jsonl`, 생성기는 `scripts/generate_tourism_noisy_realistic_chat_eval.py`, 비교 스크립트는 `scripts/compare_tourism_noisy_realistic_chat_models.py`다. 최신 결과는 `data/generated/tour_api/noisy_realistic_chat_model_compare_20260518/summary.json`이다. rule/parser 152/200, ET5 local 158/200, quickspacer 152/200, 잔여 증강 RoBERTa 170/200, ET5+RoBERTa 168/200이다. 이 셋에서는 RoBERTa 조건 보조가 가장 좋고, ET5 병행은 더 느리면서 통과율이 낮아 기본 승격하지 않는다. 남은 실패는 주로 수어/점자/보조견 희소 데이터, 멀티턴 조건 교체, 일부 negation 축약 표현이다.
- RoBERTa 조건 보조는 항상 호출하지 않고 상황별 게이트를 적용한다. 깨끗하고 조건이 명확한 입력은 rule/parser만 사용한다. rule/parser가 조건을 못 찾거나, `대중교통` 단독처럼 약한 조건만 잡히거나, 오타/무띄어쓰기/의미 우회 표현이 있으면 RoBERTa를 호출한다. 게이트 적용 뒤 noisy realistic 200건은 170/200, residual hard chat은 80/80, hard semantic chat은 39/40, medium v4 120건은 기존 100/120 수준을 유지했다. 관련 출력은 `data/generated/tour_api/eval_runs/noisy_realistic_v1_200_gated_roberta_20260518.jsonl`, `residual_hard_chat_gated_roberta_20260518.jsonl`, `hard_semantic_chat_gated_roberta_20260518.jsonl`, `medium_v4_120_gated_roberta_20260518.jsonl`이다.
