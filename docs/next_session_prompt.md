...
5. 관광 MVP 작업은 docs/GOAL.md 와 docs/accessible_tourism_mvp_plan.md 의 최신 API 메모를 먼저 확인한다.
...
- live 무장애 샘플은 `python scripts/fetch_accessible_tourism_samples.py`로 수집한다. 서울/부산/강릉 각 5건, 총 15건 수집이 2026-05-15에 성공했다.
- 수집 후 `scripts/rebuild_index.py`를 실행하면 현재 `data/raw` 기준 22개 문서가 Chroma에 색인된다.
...
- `docs/GOAL.md`: 현재 관광 MVP 목표와 API 판정 기준
- `docs/accessible_tourism_mvp_plan.md`: 무장애·가족 친화 관광 챗봇 구현 플랜
- `docs/개방데이터_활용매뉴얼(국문)/`: 국문 관광정보 서비스_GW 매뉴얼
- `docs/개방데이터_활용매뉴얼(무장애여행)/`: 무장애 여행 정보 매뉴얼
