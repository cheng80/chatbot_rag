...
단, 시군구처럼 좁은 지역을 사용자가 명시한 경우에는 추천 품질을 위해 자동 확장하지 않는다. 해당 시군구에서 3개 미만만 확인되면 부족 안내와 함께 확인된 결과만 제공한다. `근처`, `주변`, `가까운`, `인근`은 시군구 안의 가까운 후보 선호로 보고, `서울 전체로 넓혀줘`, `범위 넓혀줘`처럼 명시적 확장 표현이 있을 때만 상위 광역 지역 후보를 함께 포함한다.
...
- 지역 응답 정책은 2026-05-18에 조정했다. 예: `서울 강남구에서 휠체어 관광지 추천해줘`와 `서울 강남구 근처에서 휠체어 관광지 추천해줘`는 강남구 결과만 우선 반환하고 부족 안내를 제공한다. `서울 전체로 넓혀줘`처럼 명시적으로 요청하면 서울 범위 후보를 포함한다.
